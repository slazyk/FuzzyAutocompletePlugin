//
//  LSLTextCompletionListHeaderView.m
...
@implementation LSLTextCompletionListHeaderView {
    NSBox * _divider;
    NSTextField * _label;
    NSFont * _boldFont;
...
- (id)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        ...
        _showFilteredCount = [LSLFuzzyAutocompleteSettings currentSettings].filterByScore;
        _showTiming = [LSLFuzzyAutocompleteSettings currentSettings].showTiming;
    }
    return self;
}
...
- (void)update:(DVTTextCompletionSession *)session {
    ...
    NSString * prefix = session.lslfa_filteringQuery;
    unsigned long shownMatches = session.filteredCompletionsAlpha.count;
    unsigned long allMatches = MAX(session.lslfa_nonZeroScores, shownMatches);
    ...
    if (_showTiming) {
        status = [status stringByAppendingFormat: @" - %.2f ms", 1000 * session.lslfa_lastFilteringTime];
    }

    NSMutableAttributedString * attributed = [[NSMutableAttributedString alloc] initWithString: status];
    [attributed addAttribute: NSFontAttributeName value: _boldFont range: NSMakeRange(0, session.lslfa_filteringQuery.length)];
...
@implementation LSLTextCompletionListCornerView
